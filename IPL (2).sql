...
------------------------
select m.season,m.city,m.date,m.team1,m.team2,m.winner,m.win_by_runs
from ipl.matches m
where m.season='IPL-2017'
limit 5;


# Distinct values
-------------------
select distinct year(m.date) as 'Year of Match'
from ipl.matches m
order by 1;

select count(distinct player_of_match) as 'No of Matches'
from ipl.matches m
order by 1;
...
--------------------------------------
select distinctrow m.season, m.winner 
from ipl.matches m
order by m.season desc;

select distinct season, winner
from ipl.matches
...
------------------------------------------------
select DISTINCT m.venue,m.date 
from ipl.matches m
...
WITH t1 as (select id,season,date,city,team1,team2,winner from ipl.matches),
